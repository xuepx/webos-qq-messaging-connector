...
var qqLogin = function(qq, pwd, callback) {
  AjaxPost(qqHost, loginPath, {
    ...
    if (inResponse.returnValue == true && inResponse.responseText.indexOf("登录成功") >= 0) {
      ...
      callback({
        returnValue: true,
        sid: sidRegExp.exec(inResponse.responseText)[1]
      });
      return;
    }
    callback({
...
var qqStatusChange = function(sid, statusCode, callback) {
  //10:online
  //30:busy
  //40:hide
  AjaxPost(msgHost, msgPath + "?sid=" + sid, {
    's': statusCode,
    'aid': 'chgStatus'
  }, "", function(inResponse) {
    log("statusCode:[" + statusCode + "]" + inResponse.responseText);
    if (inResponse.responseText.indexOf("更改状态成功") > -1) {
      callback({
        returnValue: true
      });
    } else {
